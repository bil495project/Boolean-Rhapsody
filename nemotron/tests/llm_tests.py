...
from chatbot.ai_agents import (
    ...
    RecommendationExplainerAgent, RouteGenerationFormatAgent, POI_suggest_agent,
    ItineraryModificationAgent, ChatTitleAgent, POI_data_agent, POI_search_agent,
...
class TestAgentLogic:
    ...
    def test_poi_suggestion_standard(self):
        result = POI_suggest_agent()("Route at Anıtkabir")
        assert "Suggested POIs" in result

    def test_poi_suggestion_with_user_data(self):
        result = POI_suggest_agent()("Route at Anıtkabir", {"tempo": "high"})
        assert "Suggested POIs" in result
    ...
    def test_poi_data_single_match(self, mock_get):
        ...
        result = POI_data_agent()("Anıtkabir")
    ...
    def test_poi_data_no_match(self, mock_get):
        ...
        result = POI_data_agent()("FakePlace123")
    ...
    def test_explain_route_valid_stops(self, mock_get):
        """
        GeneratedRouteExplanationAgent returns a compact JSON payload.
        The human-readable 'ROUTE OVERVIEW' header is added later by the
        server's _render_route_explanation renderer — NOT by the agent itself.
        We therefore assert against the JSON structure the agent actually returns.
        """
        mock_get.return_value.status_code = 200
        ...
        # Route overview block must be present with the correct values
        overview = payload.get("route_overview", {})
        assert overview.get("total_stops") == 1
        assert overview.get("total_duration_min") == 120

        # Stops list must contain exactly one entry for "Anıtkabir"
        stops = payload.get("stops", [])
        assert len(stops) == 1
        assert stops[0].get("name") == "Anıtkabir"
        assert stops[0].get("stop") == 1
...
class TestOrchestratorUnit:
    ...
    def test_U004_intent_weather(self):
        ...
        NOTE: weatherAgent only supports CURRENT conditions; asking for
        'tomorrow' causes the LLM to correctly refuse the tool call.
        Query uses 'right now' to match the supported capability.
        """
        status, data = send_chat_request_global(
    ...
    def test_U007_intent_modify_itinerary(self):
        ...
        Uses a named-POI remove command so the LLM doesn't need to ask a
        clarifying question (e.g. 'which stop do you mean?') before calling the tool.
        """
        status, data = send_chat_request_global(
            "Remove Kocatepe Camii from my existing trip T-123."
        )
        assert status == 200
    ...
    def test_U014_weather_params(self):
        ...
        NOTE: the server returns tool_params as a raw JSON string, not a dict.
        We parse it before asserting on the location field.
        """
...
class TestLLMIntegration:
    ...
    def test_TC_LLM_I_007_weather_agent_dispatched(self):
        ...
        NOTE: weatherAgent supports CURRENT conditions only; 'tomorrow' causes the
        LLM to refuse the tool call.  Query uses 'right now' to match the capability.
        tool_params is returned as a JSON string by the server — parsed before assert.
        """
        status, data = self.send_chat_request("What is the weather in Ankara right now?")
        assert status == 200
        assert data.get("tool_used") in ["get_weather", "weather_agent"], \
