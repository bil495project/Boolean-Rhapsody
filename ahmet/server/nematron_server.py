        ...
        messages = [
            {
                ...
                "content": (
                    ...
                    "EXAMPLE: 'Start at Anıtkabir, visit Şimşek Aspava for lunch, end at a hotel, 6 stops' "
                    ...
                    "Do NOT include type keywords like 'HOTEL' here — only real place names.\n"
                    "  b) start_location — set this whenever the user indicates WHERE THE ROUTE BEGINS. "
                    "Trigger phrases: 'start at X', 'starting from X', 'I'll start at X', "
                    "'begin at X', 'beginning from X', 'first stop is X', 'depart from X'. "
                    "If the FIRST place the user mentions is clearly the starting point of their "
                    "journey, set start_location to that place name. "
                    "If they say 'start at a hotel', set start_location='HOTEL'. "
                    "CRITICAL: If start_location is a named place, it MUST also appear in named_locations.\n"
                    "  c) end_location — same rules but for the ENDING point. "
                    "Trigger phrases: 'end at X', 'finish at X', 'end the day at X', 'return to X'. "
                    "'end at hotel' or 'end the day at a hotel' → end_location='HOTEL'.\n"
                    "  d) poi_slots — INTERIOR stops only. For named places: type=PLACE with exact name. "
                    "For categories: type=TYPE with a valid poiType (KAFE, RESTAURANT, PARK, HISTORIC_PLACE, "
                    "LANDMARK, BAR). NEVER put HOTEL in poi_slots.\n"
                    "  e) stay_at_hotel — set true if user mentions hotel/accommodation/lodging.\n"
                    "  f) meal_preferences — set needsBreakfast/needsLunch/needsDinner to true as mentioned. "
                    "Meals are separate from poi_slots.\n"
                    "  g) Do NOT produce a text answer — just call the tool.\n\n"
